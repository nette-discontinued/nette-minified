...
error_reporting(E_ALL|E_STRICT);@set_magic_quotes_runtime(FALSE);@iconv_set_encoding('internal_encoding','UTF-8');extension_loaded('mbstring')&&mb_internal_encoding('UTF-8');umask(0);@header('X-Powered-By: Nette Framework');@header('Content-Type: text/html; charset=utf-8');define('NETTE',TRUE);define('NETTE_DIR',__DIR__);define('NETTE_VERSION_ID',20018);define('NETTE_PACKAGE','5.3');}namespace Nette\Diagnostics{use
Nette;interface
IBarPanel{function
getTab();function
...
__set($name,$value){ObjectMixin::set($this,$name,$value);}function
__isset($name){return
...
function
load(){if(func_num_args()>1){if(func_get_arg(1)===TRUE){return require func_get_arg(0);}foreach(func_get_arg(1)as$__k=>$__v)$$__k=$__v;unset($__k,$__v);}return require func_get_arg(0);}}}namespace Nette\Loaders{use
Nette;abstract
class
AutoLoader
extends
...
new
Nette\InvalidStateException("Unable to load class or interface '$type'.");}}}static
function
getLoaders(){return
...
static;$parts=explode(' ',$name,2);$el->setName($parts[0]);if(is_array($attrs)){$el->attrs=$attrs;}elseif($attrs!==NULL){$el->setText($attrs);}if(isset($parts[1])){foreach(Strings::matchAll($parts[1].' ','#([a-z0-9:-]+)(?:=(["\'])?(.*?)(?(2)\\2|\s))?#i')as$m){$el->attrs[$m[1]]=isset($m[3])?$m[3]:TRUE;}}return$el;}function
setName($name,$isEmpty=NULL){if($name!==NULL&&!is_string($name)){throw
new
Nette\InvalidArgumentException(sprintf('Name must be string or NULL, %s given.',gettype($name)));}$this->name=$name;$this->isEmpty=$isEmpty===NULL?isset(static::$emptyElements[$name]):(bool)$isEmpty;return$this;}function
...
new
Nette\InvalidArgumentException(sprintf('Textual content must be a scalar, %s given.',gettype($html)));}else{$html=(string)$html;}$this->removeChildren();$this->children[]=$html;return$this;}function
...
new
Nette\InvalidArgumentException(sprintf('Child node must be scalar or Html object, %s given.',is_object($child)?get_class($child):gettype($child)));}return$this;}function
...
new
Nette\Iterators\Recursor(new\ArrayIterator($this->children));}}function
...
startTag(){if($this->name){return'<'.$this->name.$this->attributes().(static::$xhtml&&$this->isEmpty?' />':'>');}else{return'';}}function
...
as$k=>$v){if($v!=NULL){$tmp[]=$v===TRUE?$k:(is_string($k)?$k.':'.$v:$v);}}if($tmp===NULL){continue;}$value=implode($key==='style'||!strncmp($key,'on',2)?';':' ',$tmp);}elseif(is_float($value)){$value=rtrim(rtrim(number_format($value,10,'.',''),'0'),'.');}else{$value=(string)$value;}$q=strpos($value,'"')===FALSE?'"':"'";$s.=' '.$key.'='.$q.str_replace(array('&',$q),array('&amp;',$q==='"'?'&quot;':'&#39;'),$value).(strpos($value,'`')!==FALSE&&strpbrk($value,' <>"\'')===FALSE?' ':'').$q;}$s=str_replace('@','&#64;',$s);return$s;}function
...
NULL;}else{$ok=self::fireLog(new\ErrorException($message,0,$severity,$file,$line));return!self::isHtmlMode()||(!self::$bar&&!$ok)?FALSE:NULL;}}static
function
...
Helpers::editorLink($row['file'],$row['line']),':',$row['line']?>
					...
					<?php if(isset($row['file'])&&is_file($row['file'])):?><a href="#" rel="netteBsSrc<?php echo"$level-$key"?>">source <abbr><?php echo$expanded!==$key?'&#x25ba;':'&#x25bc;'?></abbr></a>&nbsp; <?php endif?>
...
unlink($path);}}}namespace Nette\Config{use
Nette;use
...
setDebugMode($value=''){$this->parameters['debugMode']=is_string($value)||is_array($value)?static::detectDebugMode($value):(bool)$value;$this->parameters['productionMode']=!$this->parameters['debugMode'];return$this;}function
...
function
detectProductionMode($list=NULL){return!static::detectDebugMode($list);}}}namespace Nette\Application{use
Nette;class
Application
extends
Nette\Object{public
...
__construct(Nette\DI\Container$context){$this->context=$context;}function
...
new
Nette\Application\BadRequestException("File '$file' doesn't exist.");}$this->file=$file;$this->name=$name?$name:basename($file);$this->contentType=$contentType?$contentType:'application/octet-stream';}function
...
__construct(Nette\Application\Request$request){$this->request=$request;}function
...
new
Nette\InvalidArgumentException(sprintf('Payload must be array or object class, %s given.',gettype($payload)));}$this->payload=$payload;$this->contentType=$contentType?$contentType:'application/json';}function
...
__construct($url,$code=Http\IResponse::S302_FOUND){$this->url=(string)$url;$this->code=(int)$code;}function
...
instanceof
Container){$this->parent=$this->parent->_isCloning();if($this->parent===NULL){$this->refreshMonitors(0);}}else{$this->parent=NULL;$this->refreshMonitors(0);}}function
__sleep(){throw
new
Nette\NotImplementedException('Object serialization is not supported by class '.get_class($this));}function
__wakeup(){throw
new
...
new
Nette\InvalidArgumentException(sprintf('Component name must be integer or string, %s given.',gettype($name)));}elseif(!preg_match('#^[a-zA-Z0-9_]+\z#',$name)){throw
new
...
new
Nette\InvalidArgumentException("Component named '$name' is not located in this container.");}unset($this->components[$name]);$component->setParent(NULL);}function
getComponent($name,$need=TRUE){if(is_int($name)){$name=(string)$name;}elseif(!is_string($name)){throw
new
Nette\InvalidArgumentException(sprintf('Component name must be integer or string, %s given.',gettype($name)));}else{$a=strpos($name,self::NAME_SEPARATOR);if($a!==FALSE){$ext=(string)substr($name,$a+1);$name=substr($name,0,$a);}if($name===''){throw
new
...
new
Nette\UnexpectedValueException("Method $class::$method() did not return or create the desired component.");}return$component;}}function
...
new
InvalidLinkException(sprintf("Invalid value for persistent parameter '%s' in '%s', expected %s.",$name,$this->getName(),$type==='NULL'?'scalar':$type));}if($params[$name]===$meta['def']||($meta['def']===NULL&&is_scalar($params[$name])&&(string)$params[$name]==='')){$params[$name]=NULL;}}}function
...
new
Nette\InvalidArgumentException(sprintf('First parameter must be an array, %s given.',gettype($values)));}foreach($this->getComponents()as$name=>$control){if($control
instanceof
...
Container;$control->currentGroup=$this->currentGroup;return$this[$name]=$control;}function
offsetSet($name,$component){$this->addComponent($component,$name);}function
offsetGet($name){return$this->getComponent($name,TRUE);}function
offsetExists($name){return$this->getComponent($name,FALSE)!==NULL;}function
...
self){throw
new
Nette\InvalidStateException('Nested forms are forbidden.');}}function
...
setTranslator(Nette\Localization\ITranslator$translator=NULL){$this->translator=$translator;return$this;}function
...
new
Nette\InvalidStateException('Form is not anchored and therefore can not determine whether it was submitted.');}$data=$this->receiveHttpData();$this->httpData=(array)$data;$this->submittedBy=is_array($data);}return$this->httpData;}function
...
Nette\Http\RequestFactory;$this->httpRequest=$factory->setEncoding('UTF-8')->createHttpRequest();}return$this->httpRequest;}protected
function
getSession(){if(!$this->httpRequest){$this->httpRequest=Nette\Environment::getHttpRequest();}return
...
Link($presenter,$name.self::NAME_SEPARATOR.'submit!',array()));}if(iterator_count($this->getControls())&&$this->isSubmitted()){foreach($this->getControls()as$control){if(!$control->isDisabled()){$control->loadHttpData();}}}}parent::attached($presenter);}function
...
__construct(Nette\DI\Container$context=NULL){$this->context=$context;if($context&&$this->invalidLinkMode===NULL){$this->invalidLinkMode=$context->parameters['productionMode']?self::INVALID_LINK_SILENT:self::INVALID_LINK_WARNING;}}function
getRequest(){return$this->request;}function
...
new
Nette\InvalidStateException("Method $class::startup() or its descendant doesn't call parent::startup().");}$this->tryCall($this->formatActionMethod($this->getAction()),$this->params);foreach($this->globalParams
...
checkRequirements($element){$user=(array)$element->getAnnotation('User');if(in_array('loggedIn',$user,TRUE)&&!$this->getUser()->isLoggedIn()){throw
new
...
new
BadSignalException("The signal receiver component '$this->signalReceiver' is not ISignalReceiver implementor.");}$component->signalReceived($this->signal);$this->signal=NULL;}function
...
changeAction($action){if(is_string($action)&&Nette\Utils\Strings::match($action,'#^[a-zA-Z0-9][a-zA-Z0-9_\x7f-\xff]*\z#')){$this->action=$action;$this->view=$action;}else{$this->error('Action name is not alphanumeric string.');}}function
...
new
InvalidLinkException("No route for $presenter:$action($params)");}if($mode==='link'&&$scheme===FALSE&&!$this->absoluteUrls){$hostUrl=$refUrl->getHostUrl().'/';if(strncmp($url,$hostUrl,strlen($hostUrl))===0){$url=substr($url,strlen($hostUrl)-1);}}return$url.$fragment;}private
static
function
...
injectPrimary(Nette\DI\Container$context){$this->context=$context;}function
getContext(){return$this->context;}function
...
Nette\Callback::create($function)->invokeArgs(array_slice($key,1));});}function
...
new
Nette\InvalidStateException("Cannot load node number $nodeId.");}++$i;continue;}do{$link=$data[$i];if(isset($this->deletedLinks[$link])){continue;}elseif(!isset($node[$link])){if(self::$debug){throw
new
...
read($key){$key=urlencode($this->prefix.$key);$meta=$this->memcache->get($key);if(!$meta){return
...
remove($key){$this->memcache->delete(urlencode($this->prefix.$key),0);}function
...
new
Nette\InvalidArgumentException(sprintf('Key must be either a string or an integer, %s given.',gettype($key)));}$this->$key=$value;}function
...
function
from($time){if($time
instanceof\DateTime||$time
instanceof\DateTimeInterface){return
new
static($time->format('Y-m-d H:i:s'),$time->getTimezone());}elseif(is_numeric($time)){if($time<=self::YEAR){$time+=time();}$tmp=new
static('@'.$time);$tmp->setTimeZone(new\DateTimeZone(date_default_timezone_get()));return$tmp;}else{return
new
...
setTimestamp($timestamp){$zone=PHP_VERSION_ID===50206?new\DateTimeZone($this->getTimezone()->getName()):$this->getTimezone();$this->__construct('@'.$timestamp);$this->setTimeZone($zone);return$this;}function
getTimestamp(){$ts=$this->format('U');return
is_float($tmp=$ts*1)?$ts:$tmp;}}class
Environment{const
...
Framework{const
NAME='Nette Framework',VERSION='2.0.18',VERSION_ID=20018,REVISION='released on 2014-12-22';public
...
new
Nette\InvalidArgumentException(sprintf("The '%s' item must be scalar or array, %s given.",$prefix.$key,gettype($val)));}}}}use
...
as$query){list($sql,$params,$time,$rows,$connection,$source)=$query;$explain=NULL;if($this->explain&&preg_match('#\s*\(?\s*SELECT\s#iA',$sql)){try{$cmd=is_string($this->explain)?$this->explain:'EXPLAIN';$explain=$connection->queryArgs("$cmd $sql",$params)->fetchAll();}catch(\PDOException$e){}}$s.='<tr><td>'.sprintf('%0.3f',$time*1000);if($explain){static$counter;$counter++;$s.="<br /><a href='#' class='nette-toggler' rel='#nette-DbConnectionPanel-row-$counter'>explain&nbsp;&#x25ba;</a>";}$s.='</td><td class="nette-DbConnectionPanel-sql">'.Helpers::dumpSql(self::$maxLength?Nette\Utils\Strings::truncate($sql,self::$maxLength):$sql);if($explain){$s.="<table id='nette-DbConnectionPanel-row-$counter' class='nette-collapsed'><tr>";foreach($explain[0]as$col=>$foo){$s.='<th>'.htmlSpecialChars($col).'</th>';}$s.="</tr>";foreach($explain
as$row){$s.="<tr>";foreach($row
as$col){$s.='<td>'.htmlSpecialChars($col).'</td>';}$s.="</tr>";}$s.="</table>";}if($source){$s.=Nette\Diagnostics\Helpers::editorLink($source[0],$source[1])->class('nette-DbConnectionPanel-source');}$s.='</td><td>';foreach($params
as$param){$s.=Debugger::dump($param,TRUE);}$s.='</td><td>'.$rows.'</td></tr>';}return$this->count?'<style class="nette-debug"> #nette-debug td.nette-DbConnectionPanel-sql { background: white !important }
			...
			</table>'.(count($this->queries)<$this->count?'<p>...and more</p>':'').'</div>':'';}}}namespace Nette\Database\Drivers{use
Nette;class
MsSqlDriver
extends
Nette\Object
implements
Nette\Database\ISupplementalDriver{function
...
__construct(Nette\Database\Connection$connection,array$options){$this->connection=$connection;$charset=isset($options['charset'])?$options['charset']:'utf8';if($charset){$connection->query("SET NAMES '$charset'");}if(isset($options['sqlmode'])){$connection->query("SET sql_mode='$options[sqlmode]'");}$connection->query("SET time_zone='".date('P')."'");}function
...
getForeignKeys($table){throw
new
Nette\NotSupportedException;}}}namespace Nette\Database{use
Nette;class
Helpers{public
...
new
Nette\FileNotFoundException("Cannot open file '$file'.");}$count=0;$sql='';while(!feof($handle)){$s=fgets($handle);$sql.=$s;if(substr(rtrim($s),-1)===';'){$connection->query($sql);$sql='';$count++;}}if(trim($sql)!==''){$connection->query($sql);$count++;}fclose($handle);return$count;}}}namespace Nette\Database\Reflection{use
Nette;class
...
process($sql,$params){$this->params=$params;$this->counter=0;$this->remaining=array();$this->arrayMode='assoc';$sql=Nette\Utils\Strings::replace($sql,'~\'.*?\'|".*?"|\?|\b(?:INSERT|REPLACE|UPDATE)\b|/\*.*?\*/|--[^\n]*~si',array($this,'callback'));while($this->counter<count($params)){$sql.=' '.$this->formatValue($params[$this->counter++]);}return
...
new
Nette\InvalidArgumentException('There are more placeholders than passed parameters.');}return$this->formatValue($this->params[$this->counter++]);}else{$this->arrayMode=strtoupper($m)==='UPDATE'?'assoc':'values';return$m;}}private
function
...
instanceof\DateTime||$value
instanceof\DateTimeInterface){return$this->driver->formatDateTime($value);}elseif($value
instanceof
...
function
detectColumnTypes(){if($this->types===NULL){$this->types=array();if($this->connection->getSupplementalDriver()->isSupported(ISupplementalDriver::SUPPORT_COLUMNS_META)){$count=$this->columnCount();for($col=0;$col<$count;$col++){$meta=$this->getColumnMeta($col);if(isset($meta['native_type'])){$this->types[$meta['name']]=Helpers::detectType($meta['native_type']);}}}}return$this->types;}function
...
instanceof\Traversable){$data=iterator_to_array($data);}if($data===NULL){$data=$this->modified;}return$this->table->getConnection()->table($this->table->getName())->wherePrimary($this->getPrimary())->update($data);}function
...
as$key=>$val){if(is_int($key)){$this->where($val);}else{$this->where($key,$val);}}return$this;}if(call_user_func_array(array($this->sqlBuilder,'addWhere'),func_get_args())){$this->emptyResultSet();}return$this;}function
...
ActiveRow?$row[$column]->getPrimary():$row[$column];$keys[$key]=TRUE;}if($referenced!==NULL){$a=array_keys($keys);$b=array_keys($referenced->rows);sort($a);sort($b);if($a===$b){return$referenced;}}if($keys){$referenced=$this->createSelectionInstance($table);$referenced->where($referenced->getPrimary(),array_keys($keys));}else{$referenced=array();}}return$referenced;}function
...
new
Nette\InvalidArgumentException('Select column must be a string.');}$this->select[]=$columns;}function
...
new
Nette\InvalidArgumentException('Selection argument must have defined a select column.',0,$e);}}if($this->driverName!=='mysql'){$replace='IN ('.$clone->getSql().')';$this->parameters=array_merge($this->parameters,$clone->getSqlBuilder()->getParameters());}else{$parameter=array();foreach($clone
...
Nette\Callback($factory);try{$reflection=$factory->toReflection();}catch(\ReflectionException$e){}if(isset($e)||!$factory->isCallable()){throw
new
ServiceCreationException("Factory '$factory' used in service '$name' is not callable.");}$def->class=preg_replace('#[|\s].*#','',$reflection->getAnnotation('return'));if($def->class&&!class_exists($def->class)&&$def->class[0]!=='\\'&&$reflection
...
new
Nette\InvalidArgumentException(sprintf('Circular reference detected for variables: %s.',implode(', ',array_keys($recursive))));}else{$val=Nette\Utils\Arrays::get($params,explode('.',$part));if($recursive){$val=self::expand($val,$params,(is_array($recursive)?$recursive:array())+array($part=>1));}if(strlen($part)+2===strlen($var)){return$val;}if(!is_scalar($val)){throw
new
...
function
autowireArguments(\ReflectionFunctionAbstract$method,array$arguments,$container){$optCount=0;$num=-1;$res=array();foreach($method->getParameters()as$num=>$parameter){if(array_key_exists($num,$arguments)){$res[$num]=$arguments[$num];unset($arguments[$num]);$optCount=0;}elseif(array_key_exists($parameter->getName(),$arguments)){$res[$num]=$arguments[$parameter->getName()];unset($arguments[$parameter->getName()]);$optCount=0;}elseif($class=$parameter->getClassName()){$res[$num]=$container->getByType($class,FALSE);if($res[$num]===NULL){if($parameter->allowsNull()){$optCount++;}elseif(class_exists($class)||interface_exists($class)){throw
new
ServiceCreationException("Service of type {$class} needed by $method not found. Did you register it in configuration file?");}else{throw
new
ServiceCreationException("Class {$class} needed by $method not found. Check type hint and 'use' statements.");}}else{if($container
instanceof
...
instanceof
Html){$label->add($this->caption);}else{$label->setText($this->translate($this->caption));}return$label;}function
...
addConditionOn(IControl$control,$operation,$value=NULL){return$this->rules->addConditionOn($control,$operation,$value);}function
getRules(){return$this->rules;}function
setRequired($message=NULL){return$this->addRule(Form::FILLED,$message);}function
...
new
Nette\InvalidArgumentException("Items contain duplication for key '$key'.");}$allowed[$key]=$value;}}$this->items=$items;$this->allowed=$allowed;$this->useKeys=(bool)$useKeys;return$this;}function
...
instanceof
Nette\Utils\Html){$dest->add((string)$value2->value($key2)->selected(isset($selected[$key2])));}else{$key2=$this->useKeys?$key2:$value2;$dest->add((string)$option->value($key2)->selected(isset($selected[$key2]))->setText($this->translate((string)$value2)));}}}return$control;}}class
MultiSelectBox
extends
SelectBox{function
getValue(){return
...
sanitize($value){if($this->control->maxlength){$value=Nette\Utils\Strings::substring($value,0,$this->control->maxlength);}if(strcasecmp($this->control->getName(),'input')===0){$value=Nette\Utils\Strings::trim(strtr($value,"\r\n",'  '));}return$value;}function
...
addRule($operation,$message=NULL,$arg=NULL){if($operation===Form::FLOAT){$this->addFilter(array(__CLASS__,'filterFloat'));}elseif($operation===Form::LENGTH||$operation===Form::MAX_LENGTH){$tmp=is_array($arg)?$arg[1]:$arg;$this->control->maxlength=is_scalar($tmp)?$tmp:NULL;}return
...
TRUE;}function
getIterator(){return
new\ArrayIterator($this->rules);}function
...
new
Nette\InvalidArgumentException("Unknown operation$operation for control '{$rule->control->getName()}'.");}}private
function
...
getFile($key){return
Nette\Utils\Arrays::get($this->files,func_get_args(),NULL);}function
getFiles(){return$this->files;}function
...
new
Request($url,$query,$post,$files,$cookies,$headers,isset($_SERVER['REQUEST_METHOD'])?$_SERVER['REQUEST_METHOD']:NULL,isset($_SERVER['REMOTE_ADDR'])?$_SERVER['REMOTE_ADDR']:NULL,isset($_SERVER['REMOTE_HOST'])?$_SERVER['REMOTE_HOST']:NULL);}}class
Response
extends
Nette\Object
implements
...
redirect($url,$code=self::S302_FOUND){if(isset($_SERVER['SERVER_SOFTWARE'])&&preg_match('#^Microsoft-IIS/[1-5]#',$_SERVER['SERVER_SOFTWARE'])&&$this->getHeader('Set-Cookie')!==NULL){$this->setHeader('Refresh',"0;url=$url");return;}$this->setCode($code);$this->setHeader('Location',$url);if(preg_match('#^https?:|^\s*+[a-z0-9+.-]*+[^:]#i',$url)){$escapedUrl=htmlSpecialChars($url,ENT_IGNORE|ENT_QUOTES);echo"<h1>Redirect</h1>\n\n<p><a href=\"$escapedUrl\">Please click here to continue</a>.</p>";}}function
...
instanceof\IteratorAggregate){do{$iterator=$iterator->getIterator();}while($iterator
instanceof\IteratorAggregate);}elseif($iterator
instanceof\Traversable){if(!$iterator
instanceof\Iterator){$iterator=new\IteratorIterator($iterator);}}else{throw
new
Nette\InvalidArgumentException(sprintf('Invalid argument passed to foreach resp. %s; array or Traversable expected, %s given.',__CLASS__,is_object($iterator)?get_class($iterator):gettype($iterator)));}parent::__construct($iterator,0);}function
...
__set($name,$value){Nette\ObjectMixin::set($this,$name,$value);}function
__isset($name){return
...
HtmlNode($token->name);$htmlNode->isEmpty=in_array($this->contentType,array(self::CONTENT_HTML,self::CONTENT_XHTML),TRUE)&&isset(Nette\Utils\Html::$emptyElements[strtolower($token->name)]);$htmlNode->offset=strlen($this->output);$this->setContext(NULL);}$this->output.=$token->text;}private
function
...
new
CompileException("Found multiple macro-attributes $token->name.",0,$token->line);}$htmlNode->macroAttrs[$name]=$token->value;return;}$htmlNode->attrs[$token->name]=TRUE;$this->output.=$token->text;$context=NULL;if($token->value&&in_array($this->contentType,array(self::CONTENT_HTML,self::CONTENT_XHTML),TRUE)){$lower=strtolower($token->name);if(substr($lower,0,2)==='on'){$context=self::CONTENT_JS;}elseif($lower==='style'){$context=self::CONTENT_CSS;}}$this->setContext($token->value,$context);}private
function
...
writeAttrsMacro($code,HtmlNode$htmlNode){$attrs=$htmlNode->macroAttrs;$left=$right=array();foreach($this->macros
...
expandMacro($name,$args,$modifiers=NULL,HtmlNode$htmlNode=NULL,$prefix=NULL){if(empty($this->macros[$name])){$cdata=$this->htmlNodes&&in_array(strtolower(end($this->htmlNodes)->name),array('script','style'),TRUE);throw
new
...
new
CompileException('Missing condition in {if} macro.');}return$writer->write('if (%node.args) '.(isset($node->data->else)?'{ ob_end_clean(); ob_end_flush(); }':'ob_end_flush();').' else '.(isset($node->data->else)?'{ $_else = ob_get_contents(); ob_end_clean(); ob_end_clean(); echo $_else; }':'ob_end_clean();'));}return'}';}function
...
instanceof
Nette\Templating\IFileTemplate){if(!preg_match('#/|\\\\|[a-z]:#iA',$destination)){$destination=dirname($template->getFile()).'/'.$destination;}$tpl=clone$template;$tpl->setFile($destination);}else{throw
new
...
function
install(Latte\Compiler$compiler){$me=new
static($compiler);$me->addMacro('include',array($me,'macroInclude'));$me->addMacro('includeblock',array($me,'macroIncludeBlock'));$me->addMacro('extends',array($me,'macroExtends'));$me->addMacro('layout',array($me,'macroExtends'));$me->addMacro('block',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('define',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('snippet',array($me,'macroBlock'),array($me,'macroBlockEnd'));$me->addMacro('ifset',array($me,'macroIfset'),'}');$me->addMacro('widget',array($me,'macroControl'));$me->addMacro('control',array($me,'macroControl'));$me->addMacro('href',NULL,NULL,function(MacroNode$node,PhpWriter$writer)use($me){return' ?> href="<?php '.$me->macroLink($node,$writer).' ?>"<?php ';});$me->addMacro('plink',array($me,'macroLink'));$me->addMacro('link',array($me,'macroLink'));$me->addMacro('ifCurrent',array($me,'macroIfCurrent'),'}');$me->addMacro('contentType',array($me,'macroContentType'));$me->addMacro('status',array($me,'macroStatus'));}function
...
new
Nette\InvalidStateException("Cannot include undefined parent block '$name'.");}$block($context,$params);prev($context->blocks[$name]);}static
function
...
FALSE;}$modifiers=preg_replace('#\|noescape\s?(?=\||\z)#i','',$match['modifiers'],-1,$noescape);if($match['name']===''){$match['name']=$match['shortname']?:'=';if(!$noescape&&!$match['noescape']&&substr($match['shortname'],0,1)!=='/'){$modifiers.='|escape';}}return
...
canQuote(MacroTokenizer$tokenizer){return$tokenizer->isCurrent(MacroTokenizer::T_SYMBOL)&&(!$tokenizer->hasPrev()||$tokenizer->isPrev(',','(','[','=>',':','?','.','<','>','<=','>=','===','!==','==','!=','<>','&&','||','=','and','or','xor'))&&(!$tokenizer->hasNext()||$tokenizer->isNext(',',';',')',']','=>',':','?','.','<','>','<=','>=','===','!==','==','!=','<>','&&','||','and','or','xor'));}function
...
Compiler::CONTEXT_DOUBLE_QUOTED:if($context[1]===Compiler::CONTENT_JS){$s="Nette\\Templating\\Helpers::escapeJs($s)";}elseif($context[1]===Compiler::CONTENT_CSS){$s="Nette\\Templating\\Helpers::escapeCss($s)";}$quote=$context[0]===Compiler::CONTEXT_DOUBLE_QUOTED?', ENT_COMPAT':', ENT_QUOTES';return"Nette\\Templating\\Helpers::escapeHtml($s$quote)";case
...
Compiler::CONTENT_XML:$context=$this->compiler->getContext();if($context[0]===Compiler::CONTEXT_COMMENT){return"Nette\\Templating\\Helpers::escapeHtmlComment($s)";}case
Compiler::CONTENT_JS:case
...
NULL;}elseif(is_array($this->headers[$name])){$s='';foreach($this->headers[$name]as$email=>$name){if($name!=NULL){$s.=self::encodeHeader($name,$offset,TRUE);$email=" <$email>";}$s.=self::append($email.',',$offset);}return
ltrim(substr($s,0,-1));}elseif(preg_match('#^(\S+; (?:file)?name=)"(.*)"\z#',$this->headers[$name],$m)){$offset+=strlen($m[1]);return$m[1].'"'.self::encodeHeader($m[2],$offset).'"';}else{return
ltrim(self::encodeHeader($this->headers[$name],$offset));}}function
getHeaders(){return$this->headers;}function
...
static
function
encodeHeader($s,&$offset=0,$quotes=FALSE){if(strspn($s,"!\"#$%&\'()*+,-./0123456789:;<>@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^`abcdefghijklmnopqrstuvwxyz{|}~=? _\r\n\t")===strlen($s)){if($quotes&&preg_match('#[^ a-zA-Z0-9!\#$%&\'*+/?^_`{|}~-]#',$s)){return
self::append('"'.addcslashes($s,'"\\').'"',$offset);}return
self::append($s,$offset);}$o='';if($offset>=55){$o=self::EOL."\t";$offset=1;}$s=iconv_mime_encode(str_repeat(' ',$old=$offset),$s,array('scheme'=>'B','input-charset'=>'UTF-8','output-charset'=>'UTF-8'));$offset=strlen($s)-strrpos($s,"\n");$s=str_replace("\n ","\n\t",substr($s,$old+2));return$o.$s;}private
static
function
append($s,&$offset=0){if($offset+strlen($s)>self::LINE_LENGTH){$offset=1;$s=self::EOL."\t".$s;}$offset+=strlen($s);return$s;}}class
Message
extends
MimePart{const
...
new
Nette\InvalidStateException("Unable to send email$info.");}}}class
SmtpMailer
extends
Nette\Object
implements
...
function
write($line,$expectedCode=NULL,$message=NULL){fwrite($this->connection,$line.Message::EOL);if($expectedCode&&!in_array((int)$this->read(),(array)$expectedCode,TRUE)){throw
new
...
getTab(){if(headers_sent()&&!session_id()){return;}ob_start();?>
...
__construct(IUserStorage$storage,Nette\DI\Container$context){$this->storage=$storage;$this->context=$context;}function
getStorage(){return$this->storage;}function
...
Nette\Callback($callback);if(in_array($callback,$this->filters,TRUE)){throw
new
Nette\InvalidStateException("Filter '$callback' was registered twice.");}$this->filters[]=$callback;return$this;}function
...
Html||$s
instanceof
Form)){return$s->__toString(TRUE);}$s=(string)$s;if($quotes!==ENT_NOQUOTES&&strpos($s,'`')!==FALSE&&strpbrk($s,' <>"\'')===FALSE){$s.=' ';}return
...
new
JsonException($message);});$json=json_encode($value);restore_error_handler();if(isset($old)){ini_set('display_errors',$old);}if(PHP_VERSION_ID>=50300&&($error=json_last_error())){throw
new
...
function
decode($json,$options=0){$json=(string)$json;if(!preg_match('##u',$json)){throw
new
JsonException(static::$messages[5],5);}$forceArray=(bool)($options&self::FORCE_ARRAY);if(!$forceArray&&preg_match('#(?<=[^\\\\]")\\\\u0000(?:[^"\\\\]|\\\\.)*+"\s*+:#',$json)){throw
new
JsonException(static::$messages[JSON_ERROR_CTRL_CHAR]);}$value=json_decode($json,$forceArray);if($value===NULL&&$json!==''&&strcasecmp($json,'null')){$error=PHP_VERSION_ID>=50300?json_last_error():0;throw
new
...
new
Nette\InvalidArgumentException('Nesting level too deep or recursive dependency.');}else{$s.="\n";$list[]=$var;foreach($arr
as$k=>&$v){$s.="$space\t".self::_dump($k)." => ".self::_dump($v,$level+1).",\n";}array_pop($list);$s.=$space;}return
...
new
RegexpException("$message in pattern: $pattern");});preg_match_all($pattern,$subject,$m,($flags&PREG_PATTERN_ORDER)?$flags:($flags|PREG_SET_ORDER),$offset);restore_error_handler();if(preg_last_error()){throw
new
...
AssertionException
extends\Exception{}}namespace {Nette\Diagnostics\Debugger::_init();Nette\Utils\SafeStream::register();class_alias('Nette\Config\Configurator','Nette\Configurator');function
